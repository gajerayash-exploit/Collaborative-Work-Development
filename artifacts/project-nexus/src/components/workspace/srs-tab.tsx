...
import { isSrsLiveSyncEnabled, resolveApiUrl } from "@/lib/runtime-config";
...
function computeAudit(nodes: Node[], edges: Edge[]): AuditIssue[] {
  ...
  nodes
    ...
    .forEach((n) =>
      issues.push({
        kind: "orphan",
        nodeIds: [n.id],
        message: `"${(n.data as SRSNodeData).title}" has no connections`,
      }),
    );
  ...
  nodes
    .filter(
      (n) =>
        (n.data as SRSNodeData).priority === "critical" &&
        !hasOutgoing.has(n.id),
    )
    .forEach((n) =>
      issues.push({
        kind: "missing-critical-link",
        nodeIds: [n.id],
        message: `"${(n.data as SRSNodeData).title}" is critical but has no outgoing links`,
      }),
    );
  ...
  if (cycleSet.size > 0) {
    ...
    issues.push({
      kind: "cycle",
      nodeIds: ids,
      message: `Circular dependency between ${ids.length} node${ids.length > 1 ? "s" : ""}`,
    });
  }
    ...
    return (
      ...
      >
        ...
        <div style={{ padding: "10px 12px" }}>
          {d.locked && (
            <span style={{ color: "#000", fontWeight: 900, fontSize: 10 }}>
              🔒 LOCKED{" "}
            </span>
          )}
          <div
  ...
  return (
    <>
      <BaseEdge
        id={id}
        path={edgePath}
        style={{ stroke: VIOLET_DIM, strokeWidth: 1.5 }}
      />
      <path
        d={edgePath}
        fill="none"
...
const DEFAULT_NODES: Node[] = [
  {
    ...
    data: {
      ...
      description:
        "JWT + OAuth2 via Clerk. Session management with refresh tokens.",
      category: "auth",
      priority: "critical",
      locked: false,
    },
  },
  {
    ...
    data: {
      title: "Workspace API",
      description:
        "RESTful endpoints for CRUD workspace operations. Rate-limited.",
      category: "api",
      priority: "high",
      locked: false,
    },
  },
  {
    ...
    data: {
      ...
      description:
        "Drizzle ORM migrations. Relational data model with FK constraints.",
      category: "database",
      priority: "critical",
      locked: true,
    },
  },
  {
    ...
    data: {
      ...
      description: "Admin / Editor / Viewer permissions enforced server-side.",
      category: "constraint",
      priority: "high",
      locked: false,
    },
  },
  {
    ...
    data: {
      ...
      description:
        "Polling + optimistic updates. Activity feed with timestamps.",
      category: "functional",
      priority: "medium",
      locked: false,
    },
  },
  {
    ...
    data: {
      title: "Dashboard UI",
      description: "Kanban board, analytics charts, file manager. Responsive.",
      category: "ui",
      priority: "medium",
      locked: false,
    },
    ...
    rings = [
      {
        ...
        r1: 20,
        r2: 46,
        segments: [{ label: "Auth", color: "#f59e0b", pct: 1 }],
      },
      {
        label: "Modules",
        r1: 50,
      ...
      },
      {
        ...
        segments: [
          ...
          { label: "Migrate", color: "#22c55e55", pct: 0.1 },
...
function SRSInner({
  ...
  const [mode, setMode] = useState<"galaxy" | "radar" | "sunburst" | "audit">(
    "galaxy",
  );
  ...
  const fetchSuggestions = useCallback(async () => {
    ...
    try {
      ...
      const resp = await authenticatedFetch("/api/srs/suggest", {
        method: "POST",
  ...
  }, [nodes, edges, authenticatedFetch]);
        ...
        addEdge(
          {
            ...
            type: "pulseEdge",
            id: `ai-${Date.now()}`,
            data: { label: "", neoMode: neo },
          },
          es,
        ),
      );
      setDismissedSuggestions(
        (prev) => new Set([...prev, `${s.fromId}→${s.toId}`]),
      );
    },
    [setEdges, neo],
  );
  ...
  const generateNode = useCallback(async () => {
    ...
    try {
      ...
      const resp = await authenticatedFetch("/api/srs/generate-node", {
        method: "POST",
        body: JSON.stringify(payload),
      });
      if (!resp.ok) throw new Error("Request failed");
      const data = (await resp.json()) as {
        title: string;
        description: string;
        category: SRSNodeData["category"];
        priority: SRSNodeData["priority"];
      ...
      const cx =
        nodes.reduce((s, n) => s + n.position.x, 0) / Math.max(nodes.length, 1);
      const cy =
        nodes.reduce((s, n) => s + n.position.y, 0) / Math.max(nodes.length, 1);
      ...
      setTimeout(
        () => fitView({ nodes: [{ id: newId }], duration: 600, padding: 0.4 }),
        120,
      );
      setGenNodePrompt("");
  ...
  const focusAuditNode = useCallback(
    (nodeId: string) => {
        ...
        es.map((e) =>
          e.id === editingEdge.id
            ? {
                ...
                data: {
                  ...(e.data as object),
                  label: clear ? "" : edgeLabelDraft.trim(),
                },
              }
            : e,
        ),
  ...
  const liveSyncEnabled = isSrsLiveSyncEnabled();
  ...
  const fetchPresence = useCallback(
    async (wsId: string) => {
      ...
      try {
        const r = await authenticatedFetch(
          `/api/srs/presence/${encodeURIComponent(wsId)}`,
        );
        if (r.ok) {
    ...
    [liveSyncEnabled, authenticatedFetch],
  );
  ...
  useEffect(() => {
    ...
    const url = resolveApiUrl(
      `/api/srs/stream?workspaceId=${encodeURIComponent(workspaceId)}&senderId=${encodeURIComponent(senderId)}`,
    );
  ...
  useEffect(() => {
    ...
    syncTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        await authenticatedFetch("/api/srs/sync", {
          method: "POST",
          body: JSON.stringify({
            workspaceId,
            nodes: cleanNodes,
  ...
  auditIssues.forEach((issue) => {
    issue.nodeIds.forEach((nid) => auditKindMap.set(nid, issue.kind));
  });
  ...
  const bgStyle = neo ? { background: "#fff" } : { background: CANVAS_BG };
        ...
        {sidebarOpen && (
          <motion.div
            ...
            style={{
              flexShrink: 0,
              overflow: "hidden",
              borderRight: neo ? "3px solid #000" : `1px solid ${VIOLET_DIM}`,
                ...
                <div>
                  ...
                  <div
                    style={{
                      marginTop: 16,
                      fontSize: 10,
                      color: neo ? "#555" : "rgba(255,255,255,0.35)",
                      lineHeight: 1.6,
                    }}
                  >
                    Syntax:{" "}
                    <code style={{ color: neo ? "#000" : "#a78bfa" }}>
                      A[Label] --&gt; B
                    </code>
                    <br />
                    Supports{" "}
                      ...
                      >
                        <span
                          style={{
                            ...
                            borderRadius: "50%",
                            background: color,
                            boxShadow: neo ? "none" : `0 0 4px ${color}`,
                            flexShrink: 0,
                          }}
                        />
                        <span
                          style={{
                            fontSize: 10,
                            color: neo ? "#333" : "rgba(255,255,255,0.5)",
                ...
                <div>
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <RadarChart data={RADAR_DATA}>
                      <PolarGrid
                        stroke={neo ? "#ccc" : "rgba(139,92,246,0.2)"}
                      />
                      <PolarAngleAxis
                        dataKey="subject"
                        tick={{
                          fontSize: 9,
                          fill: neo ? "#333" : "rgba(255,255,255,0.5)",
                        }}
                      />
                      <PolarRadiusAxis
                        domain={[0, 100]}
                      ...
                      >
                        <div
                          style={{
                            flex: 1,
                            height: 3,
                            background: neo ? "#eee" : "rgba(139,92,246,0.15)",
                            borderRadius: 4,
                            overflow: "hidden",
                          }}
                        >
                        ...
                        <span
                          style={{
                            fontSize: 9,
                            color: neo ? "#555" : "rgba(255,255,255,0.4)",
                            width: 26,
                            textAlign: "right",
                          }}
                        >
                        ...
                        <span
                          style={{
                            fontSize: 9,
                            color: neo ? "#000" : "rgba(255,255,255,0.6)",
                            width: 80,
                          }}
                        >
                          {d.subject}
              ...
              {mode === "audit" &&
                (() => {
                  const byKind = {
                    ...
                    "missing-critical-link": auditIssues.filter(
                      (i) => i.kind === "missing-critical-link",
                    ),
                  ...
                  > = {
                    ...
                    "missing-critical-link": {
                      label: "Incomplete Critical",
                      color: "#f59e0b",
                      Icon: AlertTriangle,
                      desc: "Critical, no outgoing links",
                    },
                    cycle: {
                  ...
                  return (
                    <div>
                      ...
                      >
                        ...
                        <div
                          style={{
                            ...
                            borderRadius: 10,
                            background:
                              auditIssues.length > 0 ? "#ef4444" : "#22c55e",
                            color: "#fff",
                          }}
                        >
                          {auditIssues.length > 0
                            ? `${auditIssues.length} issue${auditIssues.length > 1 ? "s" : ""}`
                            : "All clear"}
                        </div>
                      </div>
                      ...
                      {auditIssues.length === 0 && (
                        <div style={{ textAlign: "center", padding: "24px 0" }}>
                          ...
                          <div
                            style={{
                              fontSize: 9,
                              color: neo ? "#666" : "rgba(255,255,255,0.3)",
                              marginTop: 4,
                            }}
                          >
                      ...
                      {(
                        ["orphan", "missing-critical-link", "cycle"] as const
                      ).map((kind) => {
                          ...
                          <div key={kind} style={{ marginBottom: 14 }}>
                            ...
                            {group
                              ...
                              .map((nodeId) => {
                                ...
                                const title =
                                  (node?.data as SRSNodeData | undefined)
                                    ?.title ?? nodeId;
                                return (
                                  <button
                                    key={nodeId}
                      ...
                      <div
                        style={{
                          marginTop: 10,
                          padding: "8px 10px",
                          background: neo ? "#eee" : "rgba(139,92,246,0.06)",
                          borderRadius: 6,
                          border: neo
                            ? "1px solid #ccc"
                            : `1px solid ${VIOLET_DIM}`,
                      ...
                      <div style={{ marginTop: 16 }}>
                        ...
                        >
                          ...
                          >
                            {suggestLoading ? (
                              <RefreshCw size={9} className="srs-spin" />
                            ) : (
                              <Sparkles size={9} />
                            )}
                        ...
                        {suggestions.length === 0 && !suggestLoading && (
                          ...
                          >
                            Refresh anytime to get live AI-powered connection
                            recommendations.
                          </div>
                        )}
                        ...
                        {(() => {
                          const CONF_COLORS = {
                            high: "#22c55e",
                            medium: "#f59e0b",
                            low: neo ? "#6b7280" : "rgba(255,255,255,0.4)",
                          };
                          ...
                          return visible.map((s) => {
                            ...
                            const confColor =
                              CONF_COLORS[s.confidence] ?? CONF_COLORS.medium;
                            return (
                              <div
                                key={key}
                                style={{
                              ...
                              >
                                ...
                                >
                                  ...
                                  <ArrowRight
                                    size={9}
                                    style={{ color: VIOLET, flexShrink: 0 }}
                                  />
                                  <span
                                    style={{
                                      fontSize: 9,
          ...
          >
            ...
            <div
              style={{
                ...
                background: neo
                  ? "#000"
                  : `linear-gradient(135deg, ${VIOLET}18, transparent)`,
              }}
            >
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                <span
                  style={{
                    ...
                    borderRadius: "50%",
                    background: VIOLET,
                    boxShadow: neo ? "none" : `0 0 6px ${VIOLET}`,
                  }}
                />
                <span
                  style={{
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  cursor: "pointer",
                  color: neo ? "#fff" : "rgba(255,255,255,0.4)",
                  padding: 2,
                }}
              >
                <X size={14} />
            ...
            <div style={{ padding: "14px 14px 10px" }}>
              <label
                style={{
                  ...
                  fontSize: 9,
                  fontWeight: 700,
                  color: neo ? "#000" : "rgba(255,255,255,0.45)",
                  textTransform: "uppercase",
                  letterSpacing: 1,
                  marginBottom: 7,
                }}
              ...
              <div
                style={{
                  fontSize: 9,
                  color: neo ? "#777" : "rgba(255,255,255,0.25)",
                  marginTop: 6,
                }}
              >
              ...
              <div style={{ marginTop: 12, marginBottom: 2 }}>
                <div
                  style={{
                    fontSize: 9,
                    fontWeight: 700,
                    color: neo ? "#000" : "rgba(255,255,255,0.35)",
                    textTransform: "uppercase",
                    letterSpacing: 1,
                    marginBottom: 6,
                  }}
                  ...
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: neo
                          ? `2px solid ${edgeLabelDraft === preset ? "#000" : "#ccc"}`
                          : `1px solid ${edgeLabelDraft === preset ? VIOLET : VIOLET_DIM}`,
                        borderRadius: neo ? 2 : 20,
                        transition: "all 0.1s",
                        boxShadow:
                          edgeLabelDraft === preset && !neo
                            ? `0 0 6px ${VIOLET}55`
                            : "none",
                      }}
                    >
          ...
          <>
            ...
            <motion.div
              ...
              style={{
                ...
                boxShadow: neo
                  ? "8px 8px 0 #000"
                  : "0 0 60px rgba(139,92,246,0.3), 0 24px 80px rgba(0,0,0,0.7)",
                overflow: "hidden",
              }}
            >
              {/* Header */}
              ...
              <div style={{ padding: "16px" }}>
                ...
                {genNodeLoading && (
                  <motion.div
                    ...
                    style={{
                      ...
                      gap: 8,
                      padding: "8px 12px",
                      background: neo ? "#eee" : "rgba(168,85,247,0.08)",
                      borderRadius: 8,
                      border: neo
                        ? "1px solid #ccc"
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontSize: 10,
                        fontWeight: 600,
                        color: neo ? "#555" : "rgba(168,85,247,0.8)",
                      }}
                    >
                ...
                <div style={{ marginTop: 14, display: "flex", gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      cursor:
                        genNodeLoading || !genNodePrompt.trim()
                          ? "default"
                          : "pointer",
                      background:
                        genNodeLoading || !genNodePrompt.trim()
                      ...
                      gap: 7,
                      boxShadow:
                        !genNodeLoading && genNodePrompt.trim() && !neo
                  ...
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      background: "transparent",
                      color: neo ? "#666" : "rgba(255,255,255,0.35)",
                      border: neo
                        ? "2px solid #ccc"
                        : `1px solid ${VIOLET_DIM}`,
                      borderRadius: neo ? 2 : 9,
                ...
                <div
                  style={{
                    marginTop: 10,
                    fontSize: 9,
                    color: neo ? "#888" : "rgba(255,255,255,0.2)",
                    textAlign: "center",
                  }}
                >
                  Press Enter to generate · Escape to close · Node auto-connects
                  to related nodes
                </div>
              </div>
            </motion.div>
                          ...
                          style={{
                            ...
                            color: active
                              ...
                              : neo
                                ...
                                : "rgba(255,255,255,0.5)",
                            border: neo
                              ? `2px solid ${active ? "#000" : "#ccc"}`
                              : `1px solid ${active ? color : VIOLET_DIM}`,
                            borderRadius: neo ? 2 : 20,
                            boxShadow:
                              active && !neo ? `0 0 8px ${color}66` : "none",
                        ...
                        return (
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: neo ? 2 : 6,
                              boxShadow:
                                active && !neo ? `0 0 8px ${color}55` : "none",
                              transition: "all 0.12s",
                            }}
                          >
                            {p}
      ...
      <div style={{ flex: 1, position: "relative", minWidth: 0, minHeight: 0 }}>
        ...
        >
          ...
          <Panel position="top-right">
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <motion.div
                ...
                title={
                  colabCount > 1
                    ? `${colabCount} people viewing this graph`
                    : "You are viewing this graph"
                }
                style={{
                  display: "flex",
                  alignItems: "center",
              ...
              <button
                ...
                style={{
                  ...
                  padding: "6px 11px",
                  cursor:
                    pdfExporting || nodes.length === 0 ? "default" : "pointer",
                  display: "flex",
                  alignItems: "center",
                  gap: 5,
                  fontWeight: 700,
          ...
          <Panel position="top-left">
            ...
            >
              {neo ? "◼ Neo-Brutalist" : "◉ Dependency Galaxy"} · {nodes.length}{" "}
              nodes
              {auditIssues.length > 0 && (
                <span
                  style={{
                ...
                >
                  ⚠ {auditIssues.length} audit issue
