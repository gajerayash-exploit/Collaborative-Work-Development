...
function PulseIndicator({ active }: { active: boolean }) {
  return (
    <span className="relative flex h-2.5 w-2.5">
      ...
      <span
        className={`relative inline-flex rounded-full h-2.5 w-2.5 ${active ? "bg-emerald-400" : "bg-muted-foreground/30"}`}
      />
    </span>
  );
}
...
export function SyncTab({ workspaceId }: { workspaceId: string }) {
  ...
  const agentCommand = `npx nexus-sync-agent --workspace ${workspaceId} --server ${getApiOrigin()}/api --token YOUR_API_TOKEN --watch ./src`;
  ...
  const liveCount =
    events?.filter((e) => Date.now() - new Date(e.pushedAt).getTime() < 30000)
      .length ?? 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 rounded-xl border bg-card p-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-foreground">
            {liveCount > 0
              ? `${liveCount} live update${liveCount !== 1 ? "s" : ""} in the last 30s`
              : "No recent activity"}
          </p>
          <p className="text-xs text-muted-foreground">
            Live-sync engine is watching for file changes
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      <div className="rounded-xl border bg-card overflow-hidden">
        ...
        <div className="p-4 space-y-3">
          <p className="text-xs text-muted-foreground">
            Run the Nexus Sync Agent locally. It watches your project files and
            pushes changes to this workspace in real time — like CTRL+S for your
            whole team.
          </p>
          <div className="rounded-lg bg-[#0d0d1a] border border-border/50 p-3 flex items-start gap-3">
            <span className="text-muted-foreground text-xs font-mono mt-0.5 select-none">
              $
            </span>
            <code className="text-xs font-mono text-violet-300 flex-1 break-all leading-relaxed">
              {agentCommand}
            </code>
            <Button
              variant="ghost"
          ...
          <p className="text-[11px] text-muted-foreground">
            ...
            with a token from your profile settings.
          </p>
        </div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 rounded-xl border border-dashed bg-muted/20">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Run the Sync Agent or simulate a push above.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {events?.map((event) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex-shrink-0 w-8 h-8 rounded-lg border flex items-center justify-center ${cfg.bg}`}
                  >
                  ...
                  <div className="flex-shrink-0 flex items-center gap-2 text-right">
                    <div>
                      <p className="text-xs text-muted-foreground">
                        {formatDistanceToNow(new Date(event.pushedAt), {
                          addSuffix: true,
                        })}
                      </p>
                      {event.pusherName && (
                        <p className="text-[10px] text-muted-foreground/70 truncate max-w-20">
                          {event.pusherName}
                        </p>
                      )}
                    </div>
