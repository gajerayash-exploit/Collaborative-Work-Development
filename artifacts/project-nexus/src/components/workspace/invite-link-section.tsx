...
export function InviteLinkSection({ workspaceId }: InviteLinkSectionProps) {
  ...
  useEffect(() => {
    authenticatedFetch(`/api/workspaces/${workspaceId}/invite-code`)
      .then(async (res) => {
        if (!res.ok) return;
  ...
  const handleRevoke = async () => {
    if (
      !confirm(
        "Revoke this invite link? Anyone with the old link won't be able to join.",
      )
    )
      return;
    setIsLoading(true);
    try {
      ...
      toast({
        title: "Error",
        description: "Could not revoke invite link.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <div className="space-y-3">
      ...
      {inviteCode === undefined ? (
        ...
        <div className="flex items-center gap-3">
          <p className="text-sm text-muted-foreground flex-1">
            No invite link active. Generate one to let anyone join.
          </p>
          <Button
            variant="outline"
            size="sm"
      ...
      <p className="text-xs text-muted-foreground">
        Anyone with this link can join as a viewer. You can revoke it at any
        time.
      </p>
    </div>
  );
