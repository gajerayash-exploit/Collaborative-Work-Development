      ...
      <div className="border-b border-sidebar-border/80 p-3">
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              className="h-auto w-full justify-between rounded-2xl border border-sidebar-border/60 bg-sidebar-accent/40 px-3 py-3 font-normal shadow-sm transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
            >
              <div className="flex items-center gap-2.5 truncate">
                ...
                <div className="min-w-0 text-left">
                  ...
                  {activeWorkspace && (
                    <p className="text-[10px] capitalize text-muted-foreground">{activeWorkspace.role}</p>
                  )}
                </div>
              </div>
          ...
          <DropdownMenuContent align="start" className="w-64 rounded-2xl border border-border/70 p-2 shadow-xl">
              ...
              {isLoading ? (
                ...
                Array.isArray(workspaces) && workspaces.map((w) => (
                  <DropdownMenuItem
                    ...
                    className="justify-between gap-2 rounded-xl px-3 py-2.5"
                  >
                    <div className="flex items-center gap-2 truncate">
                      <span className="flex-shrink-0 text-base">{getWorkspaceEmoji(w.name)}</span>
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={() => nav("/workspaces")} className="gap-2 rounded-xl px-3 py-2.5">
        ...
        <nav className="space-y-1 px-2">
          {!activeWorkspaceId && (
            <Link href="/workspaces" onClick={onClose}>
              <Button variant="ghost" className="h-10 w-full justify-start gap-2.5 rounded-xl bg-sidebar-accent/70 text-sidebar-accent-foreground shadow-sm">
                <LayoutGrid className="h-4 w-4" />
                All Workspaces
              </Button>
            </Link>
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="h-auto w-full justify-start rounded-2xl border border-sidebar-border/60 px-3 py-3 font-normal shadow-sm hover:bg-sidebar-accent">
              <Avatar className="mr-2.5 h-9 w-9 flex-shrink-0 border-2 border-border">
              ...
              <div className="flex flex-col items-start text-left truncate flex-1 min-w-0">
                <span className="w-full truncate text-sm font-semibold leading-tight">{user?.fullName || "User"}</span>
                <span className="w-full truncate text-[10px] text-muted-foreground">{user?.primaryEmailAddress?.emailAddress}</span>
              </div>
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-60 rounded-2xl border border-border/70 p-2 shadow-xl">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={() => nav("/profile")} className="gap-2 rounded-xl px-3 py-2.5">
              <UserCircle className="h-4 w-4" />
              My Profile
            </DropdownMenuItem>
            <DropdownMenuSeparator />
