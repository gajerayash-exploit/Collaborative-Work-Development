...
import {
  useWorkspaceSocket,
  type PresenceUser,
} from "@/hooks/use-workspace-socket";
import type { RtcSignal } from "@/hooks/use-huddle-rtc";
import { isWebsocketTransportEnabled } from "@/lib/runtime-config";

const TAB_CLASS =
  "data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none px-0 py-2 pb-3 font-medium text-muted-foreground data-[state=active]:text-foreground hover:text-foreground transition-colors flex-shrink-0";
...
export default function WorkspaceHubPage({ id }: { id: string }) {
  ...
  const registerRtcHandler = useCallback((fn: (msg: RtcSignal) => void) => {
    rtcHandlerRef.current = fn;
  }, []);

  const { typingUsers, sendTyping, sendMessage, myDbUserId } =
    useWorkspaceSocket({
      workspaceId: id,
          ...
          <div className="flex justify-between items-center mb-3 md:mb-4 gap-2">
            <div className="min-w-0">
              ...
              {workspace.description && (
                <p className="text-muted-foreground text-xs md:text-sm truncate hidden sm:block mt-0.5">
                  {workspace.description}
                </p>
              )}
            </div>
            <div className="flex items-center gap-1.5 flex-shrink-0">
              {websocketEnabled ? <PresenceBar users={presenceUsers} /> : null}
              {websocketEnabled ? (
                <HuddleWidget
                  workspaceId={id}
              ...
              <TabsList className="bg-transparent h-auto p-0 justify-start rounded-none gap-4 md:gap-6 w-max min-w-full">
                ...
                <TabsTrigger value="srs" className={TAB_CLASS}>
                  ...
                  {srsIssueCount > 0 && (
                    <Badge
                      variant="destructive"
                      className="ml-1.5 h-4 min-w-4 px-1 text-[9px] font-black leading-none"
                    >
        ...
        {activeTab === "srs" ? (
          ...
          <div className="flex-1 overflow-hidden p-3 md:p-5">
            <ChatTab
              workspaceId={id}
              role={workspace.role}
              wsTypingUsers={websocketEnabled ? typingUsers : undefined}
              onTyping={websocketEnabled ? sendTyping : undefined}
            />
          </div>
        ) : (
          <div className="flex-1 overflow-auto bg-muted/10">
            <div className="p-3 md:p-6 min-h-full max-w-6xl mx-auto">
              ...
              {activeTab === "files" && (
                <FilesTab workspaceId={id} role={workspace.role} />
              )}
              {activeTab === "members" && (
                <MembersTab workspaceId={id} role={workspace.role} />
              )}
              {activeTab === "tasks" && (
                <TasksTab workspaceId={id} role={workspace.role} />
              )}
              {activeTab === "vault" && (
                <VaultTab workspaceId={id} role={workspace.role} />
              )}
              {activeTab === "analytics" && <BurndownTab workspaceId={id} />}
              {activeTab === "branches" && (
                <BranchesTab workspaceId={id} role={workspace.role} />
              )}
              {activeTab === "sync" && <SyncTab workspaceId={id} />}
              {activeTab === "sandboxes" && (
                <SandboxTab workspaceId={id} role={workspace.role} />
              )}
